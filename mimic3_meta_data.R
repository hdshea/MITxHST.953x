#' ---
#' title: "SQLite and MIMIC3 definitions and useful datasets"
#' author: "H. David Shea"
...
is_icdd_hypertension_heart_and_renal_disease_with_heart_and_renal_failure <- function( code, con ) {
...
# functions for translating drug codes from DRGCODES table to common treatment categories
is_drggrp_cardiac <- function(code, type) {
    ((type == 'MS') &&
         (
             between(code, 001, 002) ||
                 between(code, 215, 238) ||
                 between(code, 242, 252) ||
                 between(code, 253, 254) ||
                 between(code, 258, 262) ||
                 between(code, 265, 267) ||
                 between(code, 280, 293) ||
                 between(code, 296, 298) ||
                 between(code, 302, 303) ||
                 between(code, 306, 313)
         )
    ) ||
    ((type == 'HCFA') &&
         (
             between(code, 103, 112) ||
                 between(code, 115, 118) ||
                 between(code, 121, 127) ||
                 (code == 129) ||
                 (code == 132) ||
                 (code == 133) ||
                 between(code, 135, 143) ||
                 between(code, 514, 518) ||
                 between(code, 525, 527) ||
                 between(code, 535, 536) ||
                 between(code, 547, 550) ||
                 between(code, 551, 558)
         )
    )
}

is_drggrp_peripheral_vascular <- function(code, type) {
    ((type == 'MS') &&
         (between(code, 299, 301))) ||
    ((type == 'HCFA') &&
        ((code == 130) || (code == 131)))
}

is_drggrp_renal <- function(code) {
    ((type == 'MS') &&
         (
             (code = 652) ||
              between(code, 656, 661) ||
              between(code, 673, 675) ||
              between(code, 682, 700)
         )
     ) ||
    ((type == 'HCFA') &&
        (
            between(code, 302, 305) ||
            between(code, 315, 333)
        )
     )
}

is_drggrp_nervous_system <- function(code, type) {
    ((type == 'MS') &&
         (
             between(code, 020, 042) ||
             between(code, 052, 103)
        )
    ) ||
    ((type == 'HCFA') &&
        (
            between(code, 1, 35) ||
            (code == 524) ||
            between(code, 528, 534) ||
            (code == 543) ||
            between(code, 559, 564) ||
            (code == 577)
        )
     )
}

is_drggrp_cerebrovascular <- function(code, type) {
    ((type == 'MS') &&
        (
            between(code, 020, 022) ||
            between(code, 034, 039) ||
            between(code, 064, 072)
        )
    ) ||
    ((type == 'HCFA') &&
        (
            ...
            (code == 524) ||
            (code == 528) ||
            between(code, 533, 534) ||
            (code == 577)
        )
    )
}

is_drggrp_copd_asthma <- function(code, type) {
    ((type == 'MS') &&
        (
        between(code, 190, 192) ||
        between(code, 202, 203)
        )
     ) ||
    ((type == 'HCFA') &&
        (
...
is_drggrp_diabetes <- function(code, type) {
    ((type == 'MS') &&
         (between(code, 637, 639))) ||
    ((type == 'HCFA') &&
         ((code == 294) || (code == 295)))
}

is_drggrp_thyroid_endocrine <- function(code, type) {
    ((type == 'MS') &&
        (
            between(code, 625, 627) ||
            between(code, 643, 645)
        )
    ) ||
    ((type == 'HCFA') &&
        (
...
is_drggrp_kidney_renal_dialysis <- function(code) {
    ((type == 'MS') &&
        (
            (code = 652) ||
            between(code, 682, 685)
        )
     ) ||
    ((type == 'HCFA') &&
        (
...
is_drggrp_liver <- function(code) {
    ((type == 'MS') &&
         (
            between(code, 420, 425) ||
            between(code, 432, 434) ||
            between(code, 441, 446)
         )
     ) ||
    ((type == 'HCFA') &&
        (
            between(code, 199, 202) ||
            between(code, 205, 208)
        )
     )
}

is_drggrp_gi_hemorrhage_ulcer <- function(code) {
    ((type == 'MS') &&
        (between(code, 377, 384))) ||
    ((type == 'HCFA') &&
        (between(code, 174, 178)))
}

is_drggrp_hiv <- function(code) {
    ((type == 'MS') &&
        (
            between(code, 969, 970) ||
            between(code, 974, 977)
        )
     ) ||
    ((type == 'HCFA') &&
        (
...
is_drggrp_leukemia_lymphoma <- function(code) {
    ((type == 'MS') &&
        (
            between(code, 820, 830) ||
            between(code, 834, 849)
        )
    ) ||
    ((type == 'HCFA') &&
        (
            between(code, 400, 414) ||
            (code == 473) ||
            (code == 492) ||
            between(code, 539, 540)
        )
    )
}

is_drggrp_cancer_lymphoma <- function(code) {
    ((type == 'MS') &&
        (
            (code = 054) ||
            (code = 055) ||
            between(code, 146, 148) ||
            between(code, 180, 182) ||
            between(code, 374, 376) ||
            between(code, 435, 437) ||
            between(code, 542, 544) ||
            between(code, 582, 585) ||
            between(code, 597, 599) ||
            between(code, 656, 658) ||
            between(code, 686, 688) ||
            between(code, 715, 716) ||
            between(code, 722, 724) ||
            between(code, 736, 741) ||
            between(code, 754, 756) ||
            between(code, 826, 830) ||
            between(code, 843, 849)
        )
    ) ||
    ((type == 'HCFA') &&
        (
            ...
            (code == 366) ||
            (code == 367) ||
            between(code, 406, 414)
        )
    )
}

is_drggrp_connective_tissue <- function(code) {
    ((type == 'MS') &&
        (between(code, 545, 547))) ||
    ((type == 'HCFA') &&
        ((code == 240) || (code == 241)))
}

is_drggrp_nutrition_metabolic <- function(code) {
    ((type == 'MS') &&
        (between(code, 640, 641))) ||
    ((type == 'HCFA') &&
        (between(code, 296, 298)))
}

is_drggrp_anemia <- function(code) {
    ((type == 'MS') &&
        (between(code, 808, 812))) ||
    ((type == 'HCFA') &&
        ((code == 395) || (code == 396) || (code == 574)))
}

is_drggrp_alcohol_drug <- function(code) {
    ((type == 'MS') &&
        (between(code, 894, 897))) ||
    ((type == 'HCFA') &&
        (between(code, 433, 437) || between(code, 521, 523)))
}

is_drggrp_coagulation_disorders <- function(code) {
    ((type == 'MS') && ((code == 813))) ||
    ((type == 'HCFA') && ((code == 397)))
}

is_drggrp_hypertensive_complicated <- function(code) {
    ((type == 'MS') &&
        ((code == 077) || (code == 078) || (code == 304))) ||
    ((type == 'HCFA') &&
        ((code == 22) || (code == 134)))
}

is_drggrp_hypertensive_uncomplicated <- function(code) {
    ((type == 'MS') &&
        ((code == 079) || (code == 305)))  |
    ((type == 'HCFA') &&
        ((code == 134)))
...
is_drggrp_psychoses <- function(code) {
    ((type == 'MS') &&
        ((code == 885))) ||
    ((type == 'HCFA') &&
        ((code == 430)))
...
is_drggrp_obesity <- function(code) {
    ((type == 'MS') &&
        (between(code, 619, 621))) ||
    ((type == 'HCFA') &&
        ((code == 288)))
}

is_drggrp_depressive_neuroses <- function(code) {
    ((type == 'MS') &&
        ((code == 881))) ||
    ((type == 'HCFA') &&
        ((code == 426)))
